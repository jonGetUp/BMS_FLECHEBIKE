 ...
 *                                  MPLAB C18 compiler
 *
 * Pascal Sartoretti    27/05/2014  Modified for XC8 and more
 * Pascal Sartoretti    11/06/2014  Send first message OK
 * Martin Aladjidi      11/03/2020  modifications for pic18f26k83 compatibility 
 ********************************************************************/
#include <xc.h>
#include "can.h"
...
//	TRISB2 = 0;
	...
	RXB0CON = 0x44;					//Receive all valid messages receive buffer overflow 
	RXB1CON = 0x40;					//writes to RXB1
	
	//Set the acceptance filters for all the filters
#ifdef ACCEPT_STANDARD_FILTER_0
	RXF0SIDL = (unsigned char)(((unsigned long)ACCEPTANCE_FILTER_0 << 5) & 0xFF);
	...
	RXF1SIDL = (unsigned char)(((unsigned long)ACCEPTANCE_FILTER_1 << 5) & 0xFF);
	...
	RXF2SIDL = (unsigned char)(((unsigned long)ACCEPTANCE_FILTER_2 << 5) & 0xFF);
	...
	RXF3SIDL = (unsigned char)(((unsigned long)ACCEPTANCE_FILTER_3 << 5) & 0xFF);
	...
	RXF4SIDL = (unsigned char)(((unsigned long)ACCEPTANCE_FILTER_4 << 5) & 0xFF);
	...
	RXF5SIDL = (unsigned char)(((unsigned long)ACCEPTANCE_FILTER_5 << 5) & 0xFF);
...
//------------------------------------------------------------------------
// MODIFIED BY SARTORETTI PASCAL
//	RXM0SIDL = (unsigned char)(((unsigned long)ACCEPTANCE_MASK_0_1 << 5) & 0xFF);
//	RXM0SIDH = (unsigned char)(ACCEPTANCE_MASK_0_1 >> 3);
 
//	RXM1SIDL = (unsigned char)(((unsigned long)ACCEPTANCE_MASK_2_5 << 5) & 0xFF);
...
//------------------------------------------------------------------------
	...
	PIR5 = 0x1C;						//Set TXB0 & TXB1 & TXB2 int flags

//Send my address to notify our presence
#ifdef MY_ADDRESS_IS_STANDARD
//	TXB0SIDL = (unsigned char)((MY_ADDRESS_IDENTIFIER << 5) & 0xFF);
...
void CANGetMessage(void)
{
	...
	for(i=0;i<14;i++)
	{
		RXMessage[RXWPtr].Data[i] = *(unsigned char *)(0x3F80+i);
	}
	RXB0FUL = 0;
}
...
char CANPutMessage(void)
{
	...
	if(TXWPtr != TXRPtr)						//If there are any more data to send
	{
		if(++TXRPtr >= TXBUF)					//then increment the TX read pointer
			TXRPtr = 0;
		ptr = (unsigned char *)0x3F80;
		for(i=0;i<14;i++)
		{
			*ptr++ = TXMessage[TXRPtr].Data[i];
		}
  		RXB0RTRRO = 1;							//TXREQ = 1;
